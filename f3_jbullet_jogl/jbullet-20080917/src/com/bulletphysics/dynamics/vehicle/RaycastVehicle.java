...
public class RaycastVehicle extends TypedConstraint {
	...
	public Transform getChassisWorldTransform(Transform out) {
		if (getRigidBody().getMotionState() != null)
		{
			getRigidBody().getMotionState().getWorldTransform(out);
			return out;
		}
	...
	public void updateFriction(float timeStep) {
		...
		for (int i = 0; i < getNumWheels(); i++) {
			...
			forwardImpulse.set(i, 0f);
                ...
                //                System.out.println("wheels on ground="+numWheelsOnGround);
		{
		...
		boolean sliding = false;
		{
			for (int wheel = 0; wheel < getNumWheels(); wheel++) {
				...
				float rollingFriction = 0f;
                                //                                System.out.println("groundObject="+groundObject);
				if (groundObject != null) {
                                    //                                                                        System.out.println("force="+wheel_info.engineForce);
		...
		// apply the impulses
		{
			for (int wheel = 0; wheel < getNumWheels(); wheel++) {
				...
				rel_pos.sub(wheel_info.raycastInfo.contactPointWS, chassisBody.getCenterOfMassPosition(tmp));
                                //                                System.out.println("forward impulse: "+ forwardImpulse.get(wheel));

				if (forwardImpulse.get(wheel) != 0f) {
					tmp.scale(forwardImpulse.get(wheel), forwardWS.get(wheel));
                                        //                                                                                System.out.println("forward impulse " + wheel + " = " + tmp + " " + chassisBody.getMotionState() + " at " + rel_pos);
