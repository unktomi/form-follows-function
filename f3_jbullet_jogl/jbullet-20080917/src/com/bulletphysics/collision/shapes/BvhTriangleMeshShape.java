...
public class BvhTriangleMeshShape extends TriangleMeshShape {
	...
	protected static class MyNodeOverlapCallback implements NodeOverlapCallback {
		...
		public void processNode(int nodeSubPart, int nodeTriangleIndex) {
			...
			IntBuffer gfxbase_ptr = data.indexbase;
			...
			for (int j = 2; j >= 0; j--) {
				int graphicsindex;
				if (false && data.indicestype == ScalarType.SHORT) {
                                    //					graphicsindex = gfxbase_ptr.getShort(gfxbase_index + j * 2) & 0xFFFF;
				}
				else {
					graphicsindex = gfxbase_ptr.get(gfxbase_index + j);
				}

				//float* graphicsbase = (float*)(vertexbase+graphicsindex*stride);
				FloatBuffer graphicsbase_ptr = data.vertexbase;
				int graphicsbase_index = graphicsindex * data.stride;

				triangle[j].set(
						graphicsbase_ptr.get(graphicsbase_index +  0) * meshScaling.x,
						graphicsbase_ptr.get(graphicsbase_index +  1) * meshScaling.y,
						graphicsbase_ptr.get(graphicsbase_index + 2) * meshScaling.z);
			}
