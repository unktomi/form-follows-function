			...
			switch (data.indicestype) {
				case INTEGER: {
					for (gfxindex = 0; gfxindex < data.numfaces; gfxindex++) {
						...
						graphicsbase = data.indexbase.get(tri_indices + 0) * data.stride;


						//triangle[0].setValue(graphicsbase[0]*meshScaling.getX(),graphicsbase[1]*meshScaling.getY(),graphicsbase[2]*meshScaling.getZ());
						triangle[0].set(
								data.vertexbase.get(graphicsbase) * meshScaling.x,
								data.vertexbase.get(graphicsbase + 1) * meshScaling.y,
								data.vertexbase.get(graphicsbase + 2) * meshScaling.z);

						//graphicsbase = (btScalar*)(vertexbase+tri_indices[1]*stride);
						graphicsbase = data.indexbase.get(tri_indices + 1) * data.stride;


						//triangle[1].setValue(graphicsbase[0]*meshScaling.getX(),graphicsbase[1]*meshScaling.getY(),graphicsbase[2]*meshScaling.getZ());
						triangle[1].set(
								data.vertexbase.get(graphicsbase + 0) * meshScaling.x,
								data.vertexbase.get(graphicsbase + 1) * meshScaling.y,
								data.vertexbase.get(graphicsbase + 2) * meshScaling.z);

						//graphicsbase = (btScalar*)(vertexbase+tri_indices[2]*stride);
						graphicsbase = data.indexbase.get(tri_indices + 2) * data.stride;
				...
				case SHORT: {
					...
					break;
                                    ...
                                    */
				default:
					assert ((data.indicestype == ScalarType.INTEGER) || (data.indicestype == ScalarType.SHORT));
			}
