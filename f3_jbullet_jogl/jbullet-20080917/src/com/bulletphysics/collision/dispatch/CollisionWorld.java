...
import com.bulletphysics.collision.narrowphase.ContinuousConvexCollision;
...
public class CollisionWorld {
    ...
    public static void rayTestSingle(Transform rayFromTrans, Transform rayToTrans,
		...
		if (collisionShape.isConvex())
		{
			...
			castResult.fraction = resultCallback.closestHitFraction;//btScalar(1.);//??
			...
			if (collisionShape.isConcave())
			{
				if (collisionShape.getShapeType()==BroadphaseNativeType.TRIANGLE_MESH_SHAPE_PROXYTYPE)
				{
					...
					Transform worldTocollisionObject = new Transform();
                                        worldTocollisionObject.inverse(colObjWorldTransform);
					Vector3f convexFromLocal = new Vector3f(convexFromTrans.origin);
                                        worldTocollisionObject.transform(convexFromLocal);
					Vector3f convexToLocal = new Vector3f(convexToTrans.origin);
                                        worldTocollisionObject.transform(convexToLocal);
					// rotation of box in local mesh space = MeshRotation^-1 * ConvexToRotation
					Transform rotationXform = new Transform();
                                        rotationXform.basis.mul(worldTocollisionObject.basis, convexToTrans.basis);
					...
					class BridgeTriangleConvexcastCallback extends TriangleConvexcastCallback
					{
						...
						BridgeTriangleConvexcastCallback(ConvexShape castShape,
                                                                                 Transform from,
                                                                                 Transform to,
                                                ...
                                                @Override
						public float reportHit(Vector3f hitNormalLocal,
					...
					BridgeTriangleConvexcastCallback tccb = new BridgeTriangleConvexcastCallback(castShape,
                                                                                                                     convexFromTrans,
                                                                                                                     convexToTrans,
                                                                                                                     resultCallback,
                                                                                                                     collisionObject,
					...
					Vector3f boxMinLocal = new Vector3f(), boxMaxLocal = new Vector3f();
				...
				} else
				{
					...
					Transform worldTocollisionObject = new Transform();
                                        colObjWorldTransform.inverse(worldTocollisionObject);
					Vector3f convexFromLocal = new Vector3f(convexFromTrans.origin);
                                        worldTocollisionObject.transform(convexFromLocal);
					Vector3f convexToLocal = new Vector3f(convexToTrans.origin);
                                        worldTocollisionObject.transform(convexToLocal);
