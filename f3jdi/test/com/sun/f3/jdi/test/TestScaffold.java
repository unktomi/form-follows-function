...
abstract public class TestScaffold extends TargetAdapter {
    ...
    static private class ArgInfo {
        ...
        String connectorSpec = "org.f3.jdi.connect.F3LaunchingConnector:";
        int traceFlags = 0;
    }

    public static String testBuildDirectory() {
        // directory where compiled org.f3.jdi.test.* classes are stored.
    ...
    public void connect(String args[]) {
        ...
        if (connection.connector().name().equals("org.f3.jdi.connect.F3LaunchingConnector") ||
    ...
    public Method findMethod(ReferenceType rt, String name, String signature) {
        if (name.indexOf('$') != -1) {
            // this is some sort of internal name, eg, f3$run
            // which are filtered out of F3ReferenceType, so we have to
            // look at the underlying ReferenceType
            rt = ((F3ReferenceType)rt)._underlying();
        }
