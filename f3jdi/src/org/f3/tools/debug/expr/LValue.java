...
abstract class LValue {
    ...
    LValue memberLValue(String fieldName, ThreadReference thread) throws ParseException {
        ...
        if ((val instanceof F3SequenceReference) &&
            "length".equals(fieldName)) {
            return new LValueF3SequenceLength((F3SequenceReference)val, thread);
        }
    ...
    LValue arrayElementLValue(ExpressionParser.GetFrame frameGetter, LValue lval) throws ParseException {
        ...
        try {
            return (arrayValue instanceof F3SequenceReference)?
                new LValueF3SequenceElement(arrayValue, index, frameGetter.get().thread()) :
...
***/
    ...
    private static class LValueF3SequenceLength extends LValue {
        final F3SequenceReference sequenceRef;
        final ThreadReference thread;

        LValueF3SequenceLength (F3SequenceReference value, ThreadReference thread) {
    ...
    private static class LValueF3SequenceElement extends LValue {
        final F3SequenceReference sequence;
        final int index;
        final ThreadReference thread;

        LValueF3SequenceElement(Value value, int index, ThreadReference thread) throws ParseException {
            if (!(value instanceof F3SequenceReference)) {
                throw new ParseException(
                       "Must be sequence type: " + value);
            }
            this.sequence = (F3SequenceReference)value;
            this.index = index;
            this.thread = thread;
        }
