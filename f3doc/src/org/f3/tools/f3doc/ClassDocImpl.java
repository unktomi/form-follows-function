...
public class ClassDocImpl extends ProgramElementDocImpl implements ClassDoc {
    ...
    public ClassDocImpl(DocEnv env, ClassSymbol sym, String documentation,
                        F3ClassDeclaration tree, Position.LineMap lineMap) {
    ...
    void addAllClasses(ListBuffer<ClassDocImpl> l, boolean filtered) {
        try {
            ...
            if (!F3docTool.isValidClassName(tsym.name.toString())) return;
    ...
    private ClassDoc searchClass(String className) {
        ...
        if (tsym.sourcefile != null) {
            ...
            F3Env<F3AttrContext> compenv = env.enter.getEnv(tsym);
    ...
    public ClassDoc[] importedClasses() {
        ...
        for (F3Tree t : compenv.toplevel.defs) {
            if (t.getF3Tag() == F3Tag.IMPORT) {
                F3Tree imp = ((F3Import) t).qualid;
                if ((F3TreeInfo.name(imp) != asterisk) &&
    ...
    public PackageDoc[] importedPackages() {
        ...
        for (F3Tree t : compenv.toplevel.defs) {
            if (t.getF3Tag() == F3Tag.IMPORT) {
                F3Tree imp = ((F3Import) t).qualid;
                if (F3TreeInfo.name(imp) == names.asterisk) {
                    F3Select sel = (F3Select)imp;
    ...
    public boolean isF3Class() {
        return env.isF3Symbol(tsym);
    }
